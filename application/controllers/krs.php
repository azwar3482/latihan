...
class krs extends CI_Controller{
    ...
    function loaddata()
    {
        ...
        $konsentrasi    =  getField('student_mahasiswa', 'konsentrasi_id', 'mahasiswa_id', $id);
        ...
        $krs            =   "select ak.krs_id,mm.kode_makul,mm.nama_makul,mm.sks,ad.nama_lengkap
                            ...
                            WHERE mm.makul_id=jk.makul_id and ad.dosen_id=jk.dosen_id and jk.jadwal_id=ak.jadwal_id and jk.tahun_akademik_id='$thn' and ak.nim='$nim' and ak.semester='".$d['semester']."'";
    ...
    echo"<tr><td colspan='3' align='right'>Total SKS</td><td>$sks</td><td colspan=2></td></tr><tr>
        <td colspan=6>
        <button onclick='loadtablemapel($id, $konsentrasi)' class='btn btn-primary btn-sm'><i class='gi gi-shopping_cart'></i> Input KRS</button> ";
    ...
    function post()
    {
        ...
        $tipe_kelas=  getField('student_mahasiswa', 'kelas', 'mahasiswa_id', $id_mahasiswa);
        $nim=  getField('student_mahasiswa', 'nim', 'mahasiswa_id', $id_mahasiswa);
        $smt=  getField('student_mahasiswa', 'semester', 'mahasiswa_id', $id_mahasiswa);

        // 1 = reguler , 2 = paket
        if ($tipe_kelas == 1) {
            // ip sebelumnya
            $smts = $smt-1;
            $cekips = getField('akademik_krs', 'ip', 'semester', $smts);
            
            $max_sks = '0'; 
            if ($cekips >= 3.00) {
                $max_sks = '24';
            } elseif ($cekips >= 2.50 && $cekips <=2.99) {
                $max_sks = '21';
            } elseif ($cekips >= 2.00 && $cekips <=2.49) {
                $max_sks = '18';
            } elseif ($cekips >= 1.50 && $cekips <=1.99) {
                $max_sks = '15';
            } elseif ($cekips <=1.49) {
                $max_sks = '12';
            }

            // cek berapa jumlah sks yang di ambil
            $ceksks = $this->db->query("SELECT c.sks FROM akademik_jadwal_kuliah AS b, makul_matakuliah AS c
                WHERE b.makul_id=c.makul_id AND b.jadwal_id='$jadwal_id'")->row();

            $cekkrs = $this->db->query("SELECT SUM(c.sks) as jsks FROM akademik_krs AS a, akademik_jadwal_kuliah AS b, makul_matakuliah AS c
                WHERE a.jadwal_id=b.jadwal_id AND b.makul_id=c.makul_id AND a.nim='$nim' AND a.semester='$smt'")->row();
            $jmlsks = $cekkrs->jsks + $ceksks->sks;
            if ($jmlsks >= $max_sks) {
                echo 'Batas SKS yang bisa anda ambil adalah '.$max_sks;
            } else {
                // simpan krs
                 $data           =   array(  'nim'=>$nim,
                                    'semester'=>$smt,
